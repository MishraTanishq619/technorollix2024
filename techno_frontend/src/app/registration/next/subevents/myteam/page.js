...
const page = () => {
  ...
  // let reqEvents = [];
  useEffect(() => {
    fetch('http://technorollix.opju.ac.in:3000/api/allEvents')
      .then((response) => response.json())
      .then((data) => setEvents(data))
      .catch((error) => console.error('Error fetching events:', error));

    fetch(
      `http://technorollix.opju.ac.in:3000/api/registeredTeam/eventId/${emailRef}`
    )
      .then((response) => response.json())
      .then((data) => {
  ...
  const validInputEmailHandler = async (email, teamId) => {
    ...
    let response = await fetch(
      'http://technorollix.opju.ac.in:3000/api/create/team-invite',
      {
        method: 'POST',
        body: JSON.stringify({
          teamId: teamId,
