...
router.post("/api/create/event", async (req: Request, res: Response) => {
	try {
		const {
			mainEventId,
			eventName,
...
router.get("/api/allEvents", async (req: Request, res: Response) => {
	try {
...
router.get("/api/allMainEvents", async (req: Request, res: Response) => {
	try {
		const events = await prisma.event.findMany({
			where: {
...
router.post(
	...
	async (req: Request, res: Response) => {
		try {
			...
			console.error(
				"Error fetching all sub events by main event ids:",
				error
			);
			res.status(500).send(
				`Error fetching all sub events by main event ids: ${error.message}`
			);
		}
	}
);
...
router.get("/api/byEventId/:eventId", async (req: Request, res: Response) => {
	const eventId = req.params.eventId;
	try {
		const events = await prisma.event.findMany({
			where: {
...
router.get(
	"/api/eventName/byEventId/:eventId",
	async (req: Request, res: Response) => {
		const eventId = req.params.eventId;
		try {
...
router.post("/api/byArrayEventId", async (req: Request, res: Response) => {
